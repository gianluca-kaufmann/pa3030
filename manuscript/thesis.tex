...
The mean LOBO PR-AUC across South American biomes is 0.233 (10 evaluated biomes;
Tropical \& Subtropical Coniferous Forests is excluded as it contains no test-period
designation events), with substantial variation across biome types. Expressed as
PR-AUC lift (PR-AUC divided by the biome's positive rate), 9 of the 10 evaluated
biomes achieve lift above the random baseline of~1.

Performance varies markedly across biomes (Figure~\ref{fig:lobo_sa}).
Temperate Broadleaf and Mixed Forests (Patagonian Chile) achieves the highest
raw LOBO PR-AUC (0.904): the biome's distinctive combination of high elevation,
low population density, and distance from infrastructure provides a covariate
signature strong enough for models trained on other biomes to generalise
effectively, even though many Patagonian designations arose through presidential
decrees and private conservation donations (notably the Tompkins Conservation land
transfers) rather than the infrastructure-and-land-value gradients that dominate
elsewhere. Temperate Grasslands, Savannas and Shrublands yields the highest
PR-AUC lift (843$\times$, against a 0.09\% positive rate): the model identifies
rare designation events in this biome with high precision, consistent with the
distinctively low economic value and high remoteness of Patagonian steppe
landscapes targeted for protection.

By contrast, Flooded Grasslands and Savannas achieves below-random lift
(0.6$\times$), the only biome in this analysis that falls beneath the random
classifier baseline. This biome spans Amazonian v\'{a}rzea and the Pantanal,
where designation dynamics are governed by seasonal flood regimes and
site-specific conservation processes rather than the static geospatial gradients
the feature set can observe. These contrasts are informative: they identify
ecosystems where dynamic land-cover and water-regime covariates would be required
to improve cross-biome generalisation.
...
The dashed vertical line marks each biome's positive rate (random-classifier baseline);
bars extending beyond it indicate genuine predictive lift above chance.
Temperate Broadleaf \& Mixed Forests achieves the highest raw PR-AUC (0.904).
Temperate Grasslands, Savannas \& Shrublands achieves the highest lift
(843$\times$) owing to its very low positive rate (0.09\%). Flooded Grasslands \&
Savannas is the only biome with below-random lift (0.6$\times$).}
\label{fig:lobo_sa}
\end{figure}
...
\noindent\textbf{[TODO: the United States and Southeast Asia LOBO tables
(Tables~\ref{tab:lobo_usa} and~\ref{tab:lobo_seasia}) must be regenerated once
the corrected Euler results are available. The South America table has been updated
with post-fix results (April 2026).}]
