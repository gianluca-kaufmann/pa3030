...
land-use change, overexploitation, climate change, invasive species, and pollution —
compound ecological pressure and degrade the capacity of ecosystems to sustain human
...
dataset at continental scale — across three regions totalling over 700 million
...
tropical and subtropical forests, mangroves, and montane ecosystems — partly mirrors
...
crop and pasture pixels in proportion to their MODIS land-cover area. This
...
recognised limitation, discussed further in Section~\ref{sec:limitations}.
...
is justified on three grounds. First, data compatibility: the key predictors — WDPA
...
sensitive than PR-AUC under extreme class imbalance — it is dominated by the large
...
well or poorly — which biomes produce the clearest predictive signal versus where
...
— same hyperparameters, same features, same class-weighting scheme, and same
...
% NEEDS HUMAN REVIEW: Consider adding a bar chart of annual SA transition rates (2000-2019) as a supplementary figure if space allows.
...
\label{fig:pr_sa}
\end{figure}
Figure~\ref{fig:pr_sa} shows the Precision--Recall curve for the LightGBM model. As
the classification threshold decreases from right to left, precision (fraction of flagged
pixels that actually transition) falls while recall (fraction of all transitions flagged)
rises. The curve lies far above the dashed baseline throughout, confirming that the
model's advantage over random selection is not an artefact of a single threshold but
holds across the entire operating range. A cumulative gains chart
(Figure~\ref{fig:cumgains_sa}, Appendix~\ref{app:supp_figures}) provides a complementary
view from the perspective of recall: it shows the fraction of all test-period transitions
captured as a function of the fraction of the risk set screened.
...
calibration are the more actionable metrics — they measure how well the model identifies
...
year off still receives no credit for that positive. For a policy application —
...
event in the test period, the metric checks whether that event's location falls
within the top-1\% risk zone at any point during the three test years. Under this
aggregation, the LightGBM model captures 90.4\% of all protected-area establishments
during the 2017--2019 test window within its highest-risk 1\% of pixels
(approximately 156,000~km$^2$ out of 15.7~million~km$^2$ eligible). In other words,
screening fewer than 1\% of candidate parcels would have flagged nine out of every
ten actual new protected areas before they were designated. This result demonstrates
that the model has strong spatial foresight: the moderate pixel--year PR-AUC reflects
the inherent difficulty of predicting the exact year of a politically-driven
administrative decision, not a failure of spatial targeting. The practical implication
is direct: a risk analyst using the model's top-1\% risk zone as a geographic filter
would capture the vast majority of future designation events well in advance.
...
that feature's SHAP value — how much it pushed the prediction above (positive) or
...
Features are sorted by mean absolute SHAP value (overall importance). Each dot
represents one pixel--year observation; horizontal position is the SHAP value for
that feature; colour indicates the raw feature value (red = high, blue = low).
Feature labels: \texttt{GSN\_b2\_smooth16} = High Biodiversity Areas (16 km
smoothing); \texttt{GSN\_b3\_smooth16} = Intact Wilderness Areas (16 km smoothing);
\texttt{policy\_b4} = WGI Rule of Law; \texttt{WorldClim\_b14} = Precipitation of
Driest Month; \texttt{GSN\_b2\_smooth64} = High Biodiversity Areas (64 km smoothing);
\texttt{dist\_indigenous} = Distance to indigenous territories;
\texttt{dist\_wdpa} = Distance to nearest PA; \texttt{economic\_value\_lag1} =
Agricultural land value; \texttt{policy\_b2} = V-Dem Liberal Democracy Index;
\texttt{elevation\_b2\_smooth16} = Elevation (16 km smoothing).}
\label{fig:shap_sa}
\end{figure}

The SHAP rankings reveal a nuanced picture that partly confirms and partly qualifies
the simple opportunity-cost narrative. The evidence points to three co-determining
channels:
...
PA designations during the test period (2017--2019). High-risk pixels form spatially
...
Chile's southern regions. The strong spatial correspondence between predicted
high-risk zones and realised designations — visible in the figure and quantified
by the 90.4\% event-level recall — confirms that the model produces geographically
meaningful risk concentrations, not merely a good score on abstract metrics.
...
\end{figure}
% [HUMAN ACTION NEEDED: Figure size/legend requires re-running the results script with improved parameters]
...
\end{figure}
% [HUMAN ACTION NEEDED: Figure size/legend requires re-running the results script]
...
a held-out one (see Section~\ref{sec:lobo} for the methodological rationale and
the justification for using ROC-AUC as the LOBO metric). The mean LOBO ROC-AUC
across all nine South American biomes is 0.808, confirming that the model captures
continent-wide structural patterns, not only biome-specific correlations.

Performance varies across biomes (Figure~\ref{fig:lobo_sa}), and two patterns stand
out. Deserts and Xeric Shrublands (principally the Atacama) achieve a near-perfect
LOBO ROC-AUC of 0.995: the distinctive combination of extreme aridity, elevation,
and low population density is a strong enough signature that a model trained on other
biomes can reliably rank Atacama pixels correctly. By contrast, Temperate Broadleaf
and Mixed Forests (Patagonian Chile) achieves a LOBO ROC-AUC of 0.491 — essentially
random. This is consistent with institutional drivers in Patagonia that are not
represented in the model's feature set: much of the Chilean national park network in
this biome was established through presidential decree and private conservation
transactions (e.g.\ the Tompkins Conservation land donations), which do not follow
the accessibility and land-value gradients that dominate designation elsewhere.
Mediterranean Forests, Woodlands and Scrub (central Chile) also underperform
(ROC-AUC 0.701); this biome has a very low positive rate, which makes ROC-AUC
estimates less stable for the small number of test-period events.

These biome-level failures are informative rather than simply disappointing: they
...
These are genuine institutional findings, not modelling artefacts: PA designation in
Pacific Northwest conifer forests and in BLM desert land involves complex negotiations
over federal land allocation that are not captured by the geospatial covariates
available at global scale. Land tenure — whether a parcel is federally owned versus
privately held — is the primary predictor of US designation, and it is absent from
the model's feature set. The weak LOBO scores in these biomes thus reveal a real
data gap rather than a model deficiency.
...
The LOBO analysis for Southeast Asia yields a mean ROC-AUC of 0.883 across evaluated
biomes (Figure~\ref{fig:lobo_seasia}, Appendix~\ref{app:supp_figures}), a more
homogeneous performance profile than South America. All biome folds with sufficient
positive examples — Tropical and Subtropical Moist Broadleaf Forests (0.826), Tropical
Dry Broadleaf Forests (0.818), Temperate Conifer Forests (0.822), Mangroves (0.957),
and Tropical Coniferous Forests (0.991) — exceed ROC-AUC 0.8. This consistency
indicates that pan-regional ecological gradients (elevation, forest cover, remoteness)
carry predictive signal across biome contexts. Several biome folds contain too few
...
a farm boundary, a watershed, a lending portfolio — simply by averaging or summing
...
\label{sec:results_ecological_overlap}

% [TODO HUMAN: Insert results here from the GSN high-biodiversity and climate-stabilisation area overlap analysis.
%  These results should address whether predicted high-risk zones overlap with ecologically important areas,
%  and whether historical designations have targeted ecologically priority land.
%  The data/code for this analysis already exists in the outputs directory.]
...
here — calibrated probabilities capture only the designation risk side — but
...
or emerging nature-related financial disclosure requirements \citep{tnfd2023} —
are not modelled. Calibrated probabilities should be interpreted as conditional
...
years in some jurisdictions \citep{bingham2019}. The five-year lookahead window reduces sensitivity to
...
\paragraph{Proximity-to-PA feature and administrative processes.}
...
\paragraph{Temporal weighting sensitivity.}
Observations in later training years receive higher weight (Section~\ref{sec:splits}).
The choice of weighting ramp is a modelling decision; the robustness of key results to
removing temporal weights or using alternative weighting schemes has not been formally
tested, as this would require full HPC re-training runs beyond the scope of this thesis.
If the post-2010 acceleration in designation pace is a temporary policy phase rather
than a persistent structural shift, temporal weighting may cause the model to
over-extrapolate recent dynamics.

\paragraph{Absence of formal ablation study.}
The relative contribution of each predictor group — governance indicators, biodiversity
layers, economic value, climate variables, path dependence — has been characterised
through SHAP values, but a formal ablation study (removing each group in turn and
measuring the change in PR-AUC) has not been conducted. Ablation would provide stronger
evidence that the individual predictor groups independently add predictive value rather
than acting as proxies for one another. This remains a priority for future work.
...
\paragraph{False positives and the right-prediction-wrong-window problem.}
Pixels assigned high predicted probability that are \emph{not} designated within the
...
(Appendix~\ref{app:backtest}) reveals that a substantial share of these pixels cluster
along known designation frontiers — in the same geographic corridors that received PAs
in adjacent time windows. Many false positives are therefore better characterised as
``right prediction, wrong window'': the model correctly identifies them as elevated-risk
zones, but the designation is delayed beyond the five-year horizon. This is an inherent
...
designation candidates rather than noise.
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth, keepaspectratio]{../outputs/south_america/results/forward/lgbm/hotspot_maps.pdf}
\caption{Hotspot zoom-ins for South America (LightGBM forward model, BAU scenario).
Top row: predicted probability of designation by 2030 (same colour stretch as the
continental map). Bottom row: BAU designation zone (red), with existing protected areas
(2024) shown in green and country borders in grey. Left panel shows the Colombian
Amazon cluster (C1), the highest-ranked BAU hotspot; right panel shows the central
Chilean cluster (C5), illustrating the Bare/Other land-cover regime characteristic of
Andean foothills designations. Both clusters illustrate the model's ability to identify
spatially coherent designation pressure at sub-national scale.}
