...
\textbf{Ecological conservation priority.} The top-ranked features by mean
$|\text{SHAP}|$ include High Biodiversity Areas and Intact Wilderness at
neighbourhood-scale smoothings (16~km and 64~km), with positive SHAP values
confirming that pixels embedded in globally significant biodiversity and wilderness
areas attract designation \citep{mouillot2024}. Smoothed versions dominate over
raw pixel-level indicators, consistent with designation being a landscape-scale
process responding to the \emph{ecological neighbourhood}.

\textbf{Institutional capacity.} WGI Rule of Law (\texttt{policy\_b4}), V-Dem
Liberal Democracy Index (\texttt{policy\_b2}), and WGI Government Effectiveness
(\texttt{policy\_b3}) rank among the top individual features. Their positive SHAP
values indicate that stronger governance raises designation probability, consistent
with institutional capacity translating international commitments into on-the-ground
action \citep{watson2014}.

\textbf{Economic opportunity cost.} Agricultural land value has predominantly
negative SHAP values: high-value agricultural pixels are systematically suppressed
from high-risk zones, consistent with the opportunity-cost mechanism \citep{joppa2009}.
Distance to indigenous territories shows mixed-sign SHAP, reflecting their dual
role as both designation anchors and buffers. Infrastructure variables (night-time
lights, roads, population density) show consistent negative SHAP, confirming that
economic activity deters designation.

Designation is most probable where ecological priority is high, governance is strong,
and economic opportunity cost is low --- the three conditions rarely co-occur with
high agricultural value, explaining the historical bias toward marginal land
\citep{joppa2009, venter2018}. SHAP dependence plots for the four most important
features are provided in Appendix~\ref{app:shap_dep}.
