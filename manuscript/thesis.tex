...
\newcommand{\thesistitle}{Predicting Protected Area Designations and Mapping Transition Risk under 30$\times$30}
...
policy problem. For conservation policymakers, the answer determines whether the
...
data snapshot. In South America, the framework achieves strong out-of-sample
...
The biodiversity and climate crises are increasingly intertwined. Carbon-dense
...
characteristic of protected relative to unprotected land. %TODO: Seems complicated, explain more intuitively
...
expected loss in credit risk. %TODO: Find a relevant recent source for this explanation.
...
given that no transition has occurred yet. Unlike parametric hazard models, the tree-based approach imposes no functional form
and handles complex interactions among the 73 engineered covariates without prior
specification. The target is a \emph{five-year cumulative} transition indicator
...
Raw model outputs are scores, not calibrated probabilities: a score of 0.7 indicates
only that the pixel ranks above most others, not that its true designation probability
is 70\%. Portfolio screening and stress-testing require interval-scale probabilities
that admit direct comparison across assets. The gap arises because class-weighting
(Equation~\ref{eq:spw}) reshapes the effective class distribution, causing the model
to assign higher scores than the true minority-class probability warrants.
...
probability accuracy. The post-calibration ECE of 0.0081 (0.81 pp absolute deviation on a 0.8\% base rate)
is expected for a rare-event model; reliability diagrams
(Appendix~\ref{app:calibration}) confirm substantially better calibration within
the informative upper tail.
...
\end{figure}
Figure~\ref{fig:pr_gains_sa} shows the PR curve lying far above the random baseline
throughout (confirming model advantage is not threshold-specific) and the cumulative
gains chart (top 1\% of pixels captures $\approx$66.8\% of transitions,
Lift $66.8\times$).
...
$=$ increases predicted probability); colour encodes feature value (red~=~high,
blue~=~low).
...
establishment---the model identifies where governments acted, not merely which areas
...
Land that is both projected for designation and classified as biodiversity-priority
covers 145,154~km$^2$. The overlap with climate-stabilisation
