...
formal transition indicator introduced in Section~\ref{sec:methods}.
...
twice those of pasture \citep{richards2014}. Comparable multi-country land-value surveys for Southeast Asia at the required spatial resolution are not available, so the same 3:1 weight is applied there as an approximation; this is noted as a limitation in Section~\ref{sec:discussion_limitations}.
This
...
forest protection and formal PA networks, a pattern confirmed in cross-country panel analysis covering 101 countries from 1991 to 2018 \citep{wangsun2022}.
...
\citep{greenhill2024, mouillot2024}. Web Mercator introduces latitude-dependent areal distortion that increases away from the equator. Since all input layers share the same projection, this distortion affects every feature in a given cell consistently, which preserves relative covariate signals used by tree-based models. Distance-based features may be slightly overstated at higher latitudes; this is a known trade-off of Mercator-based pipelines and was accepted in favour of consistent alignment across the multi-source input layers.
...
which schedule independent parallel workloads (e.g.\ one array task per Leave-One-Biome-Out (LOBO; see Section~\ref{sec:lobo}) biome fold or per forward-prediction scenario) across the cluster's compute nodes, reducing
...
\begin{figure}[H]
  ...
  \end{subfigure}
  \caption{South America LightGBM transition-risk maps. \textbf{(a)} Top 1\% of predicted pixels (Recall@1\% = 77.4\%, Precision@1\% = 79.8\%), overlaid with observed PA establishments within the five-year prediction window. \textbf{(b)} Continuous calibrated five-year designation probabilities, showing a strongly right-skewed distribution with a small upper tail concentrating substantially elevated risk. Probabilities reflect covariate values at the end of the test period.}
...
designation events), with substantial variation across biome types; the mean is
dominated by the two temperate biomes (Temperate Broadleaf 0.904, Temperate Grasslands
0.738), while the median across all ten biomes is 0.096. Expressed as PR-AUC lift
(PR-AUC divided by the biome's positive rate), 9 of the 10 evaluated biomes achieve
lift above the random baseline of~1.

Performance varies markedly across biomes.
Temperate Broadleaf and Mixed Forests (Patagonian Chile) achieves the highest
raw LOBO PR-AUC (0.904): high elevation, low population density, and remoteness
provide a covariate signature strong enough for models trained on other biomes to
generalise effectively. Temperate Grasslands, Savannas and Shrublands yields the
highest PR-AUC lift (843$\times$): with a near-zero positive rate (0.09\%) the
lift ratio is amplified, but absolute performance is also strong (PR-AUC 0.738,
78\% recall at the top 1\%).
...
ROC-AUC 0.19. Designation here is governed by seasonal flood regimes and
site-specific processes not recoverable from static continental covariates.
...
This transfer failure is itself a primary result. It establishes the
\emph{institutional specificity} of protected-area designation: the spatial pattern of
PA creation is not derivable from physical geography alone but is determined by the
interaction of geographic covariates with continent-level governance structures,
...
mapping from features to political feasibility differs fundamentally across governance
contexts. Even country-level institutional indicators (WGI Rule of Law, V-Dem) cannot
bridge this gap, as the specific form of institutional action varies too much to be
proxied by composite scores.

Each regional model has learned the specific feature-to-designation mapping prevailing
in its own governance context. The non-transportability of that mapping across continents
confirms that the models capture institutionally conditioned decision-making, not merely
universal biophysics. A practical corollary is that reliable transition-risk assessments
require region-specific models: a single global score would conflate governance contexts
with fundamentally different feature-to-designation relationships.
...
backtesting validation in Appendix~\ref{app:backtest} provides additional
...
protected/unprotected classification \citep{fitzsimons2025, maxwell2020}.
...
($16\times16$, $64\times64$~km for dynamic variables;
$4\times4$, $16\times16$, $64\times64$~km for static terrain),
...
\midrule
%% ---- Climate ---------------------------------------------------------------
\multicolumn{3}{l}{\textit{Climate — WorldClim V1 bioclimatic normals (1960--1991; static)}} \\
\midrule
...
\texttt{GSN\_b5}          & Static & Terrestrial ecoregion code (categorical, WWF via GSN) \\
...
\texttt{wildfire\_b1}     & Dynamic & Annual sum of burn dates (MODIS MCD64A1) \\
...
\midrule
\texttt{WDPA}             & Dynamic & Current-year protected area status (binary, WDPA) \\
