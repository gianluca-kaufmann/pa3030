...
\begin{longtable}{%
  >{\raggedright\arraybackslash\footnotesize}p{0.19\linewidth}%
  >{\raggedright\arraybackslash\footnotesize}p{0.46\linewidth}%
  >{\raggedright\arraybackslash\footnotesize}p{0.28\linewidth}%
}
...
twice those of pasture \citep{richards2014}.
This
...
environmental protection in several of the study regions.
...
The mean LOBO PR-AUC across South American biomes is \textbf{[TODO: update after
corrected Euler results]}, with substantial variation across biome types; expressed
as PR-AUC lift (PR-AUC divided by the biome's positive rate), the majority of
biomes achieve lift well above the random baseline of~1.
...
The dashed vertical line marks each biome's positive rate (the random-classifier
baseline); bars extending further right indicate genuine predictive lift above chance.
The Deserts \& Xeric Shrublands biome achieves the highest raw PR-AUC owing to
its distinctively high positive rate, while biomes such as Mangroves achieve
comparable or higher \emph{lift} (PR-AUC divided by positive rate) despite a lower
absolute PR-AUC. \textbf{[TODO: update numbers after corrected Euler results.]}}
\label{fig:lobo_sa}
\end{figure}
...
LOBO PR-AUC of \textbf{[TODO: update after corrected Euler results]} across
evaluable biomes. All biome folds achieve near-zero PR-AUC lift, confirming
that the model's discrimination is highly biome-specific and does not transfer
across ecological contexts. This is a genuine institutional finding rather
...
The LOBO analysis for Southeast Asia yields a mean PR-AUC of \textbf{[TODO: update
after corrected Euler results]} across evaluable biomes
(Figure~\ref{fig:lobo_seasia}, Appendix~\ref{app:supp_figures}), with strong
heterogeneity. Temperate Conifer Forests and Tropical Coniferous Forests
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \textit{\small Blue: designated \& priority; Red-orange: priority not designated; Gold: designated, not priority}}
...
models); Tables~\ref{tab:usa_metrics} and \ref{tab:seasia_metrics} report the
benchmark metrics for USA and SE Asia respectively.

\noindent\textbf{[TODO: all three LOBO tables below must be regenerated after the
corrected Euler results are synced. The numbers currently shown are from a run with
a broken biome raster (April 2026 fix pending) and will change. Two additional SA
biome rows will also be added.}]

\begin{table}[H]
\centering\small
\caption{South America: Per-biome LOBO PR-AUC and PR-AUC lift (LightGBM).
Lift = PR-AUC / positive rate; lift~$=1$ equals a random classifier.
\textbf{[TODO: regenerate from corrected Euler results; two additional biome rows
(Temperate Grasslands \& Shrublands; Tropical \& Subtropical Coniferous Forests)
will be added and all values will change.]}}
\label{tab:lobo_sa}
\begin{tabular}{lrrrr}
\toprule
\textbf{Biome} & \textbf{N pos.} & \textbf{Pos.\ rate} & \textbf{PR-AUC} & \textbf{Lift} \\
\midrule
Deserts \& Xeric Shrublands                   & 109,678 & 10.4\% & 0.980 & 9.4$\times$ \\
Flooded Grasslands \& Savannas                &   1,425 &  2.7\% & 0.219 & 8.1$\times$ \\
Mangroves                                     &   1,457 &  0.2\% & 0.215 & 108$\times$ \\
Mediterranean Forests, Woodlands \& Scrub     &     442 &  0.8\% & 0.037 & 4.6$\times$ \\
Montane Grasslands \& Shrublands              &   5,184 &  0.07\% & 0.002 & 2.9$\times$ \\
Temperate Broadleaf \& Mixed Forests          &     889 &  2.0\% & 0.035 & 1.8$\times$ \\
Tropical \& Subtropical Dry Broadleaf Forests & 10,328 &  0.4\% & 0.232 & 58$\times$ \\
Tropical \& Subtropical Grasslands \& Savannas &  4,184 &  0.4\% & 0.118 & 30$\times$ \\
Tropical \& Subtropical Moist Broadleaf       & 62,359 &  0.5\% & 0.107 & 21$\times$ \\
\midrule
\textbf{Mean}                                &         &        & \textbf{0.216} & \textbf{[TODO]} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{[TODO: regenerate from corrected Euler results.]}}
\label{tab:lobo_usa}
\begin{tabular}{lrrrr}
\toprule
\textbf{Biome} & \textbf{N pos.} & \textbf{Pos.\ rate} & \textbf{PR-AUC} & \textbf{Lift} \\
...
\midrule
\textbf{Mean (evaluated)} & & & \textbf{0.001} & \textbf{[TODO]} \\
\bottomrule
\end{tabular}
\end{table}
...
Lift~$=1$ equals a random classifier. Positive rates for SE Asia biomes were
missing from pre-corrected outputs and will be populated from the rerun.
\textbf{[TODO: regenerate from corrected Euler results; add positive rates
and recompute lift.]}}
...
\textbf{Biome} & \textbf{N pos.} & \textbf{Pos.\ rate} & \textbf{PR-AUC} & \textbf{Lift} \\
\midrule
Mangroves                                           &    46 & [TODO] & 0.007 & [TODO] \\
Temperate Conifer Forests                           & 4,284 & [TODO] & 0.634 & [TODO] \\
Tropical \& Subtropical Coniferous Forests          &    21 & [TODO] & 0.603 & [TODO] \\
Tropical \& Subtropical Dry Broadleaf Forests       & 17,702 & [TODO] & 0.041 & [TODO] \\
Tropical \& Subtropical Moist Broadleaf Forests     & 51,475 & [TODO] & 0.066 & [TODO] \\
Montane Grasslands; Temperate Broadleaf; Trop.\ Grasslands & 0 & -- & N/A & N/A \\
\midrule
\textbf{Mean (evaluated)} & & & \textbf{0.270} & \textbf{[TODO]} \\
\bottomrule
\end{tabular}
\end{table}
...
Biomes with zero test-period designations are excluded. LOBO PR-AUC lift
(PR-AUC divided by each biome's positive rate) is near or below~1 across all
evaluated biomes, confirming that the USA model relies on federal public-land
tenure patterns that do not transfer across biome boundaries.
\textbf{[TODO: regenerate from corrected Euler results.]}}
\label{fig:lobo_usa}
\end{figure}
...
Biomes with zero test-period designations are excluded. Conifer-dominated biomes
(Temperate Conifer Forests; Tropical Coniferous Forests) achieve high PR-AUC lift,
while moist tropical biomes show near-random lift, consistent with the
institutional interpretation in the main text.
\textbf{[TODO: regenerate from corrected Euler results.]}}
