#!/usr/bin/env python3
"""Stage 0b (RF): Deployment Model Training — South East Asia RandomForest (annual hazard, 2001-2024).
...
Key differences from model3_LGBM_deployment.py:
  ...
  - Negatives optionally capped via MAX_NEG_TRAIN env var (following model3_RF pattern)
...
Outputs:
  data/se_asia/ml/models/model3_rf_deployment_{timestamp}.pkl
  ...
  outputs/se_asia/results/forward/model3_rf_deployment_{timestamp}.txt
...
def resolve_best_params_json() -> Optional[Path]:
    ...
    if scratch is not None:
        candidates += [
            scratch / "scripts/regions/se_asia/5_training/rf_best_params.json",
            scratch / "rf_best_params.json",
        ]
    candidates += [
        repo_root / "scripts/regions/se_asia/5_training/rf_best_params.json",
    ]
    for cand in candidates:
        if cand.exists():
            return cand
...
def main() -> None:
    ...
    print("=" * 70)
    print(f"MODEL 3 RF — DEPLOYMENT MODEL (annual hazard, {DEPLOY_TRAIN_YEARS[0]}-{DEPLOY_TRAIN_YEARS[1]})")
    print("=" * 70)
    ...
    check_feature_denylist(feature_cols, context="se_asia/rf/deployment")
    ...
    model_path = model_dir / f"model3_rf_deployment_{timestamp}.pkl"
...
if __name__ == "__main__":
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = output_dir / f"model3_rf_deployment_{timestamp}.txt"
    tee = Tee(log_path)
    sys.stdout = tee
    try:
        main()
