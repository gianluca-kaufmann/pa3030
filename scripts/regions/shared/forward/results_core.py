...
def create_country_breakdown(
    ...
    if world_gdf is None:
        print("  Skipping country breakdown — naturalearth boundaries not available")
        return pd.DataFrame()
    ...
    try:
        ...
        csv_path = output_dir / "forward_country_breakdown.csv"
...
def create_biome_breakdown(
    ...
    try:
        ...
        csv_path = output_dir / "forward_biome_breakdown.csv"
...
def create_gap_analysis(
    ...
    for ext in ["pdf", "png"]:
        p = output_dir / f"forward_gap_analysis.{ext}"
        plt.savefig(p, dpi=MAP_DPI, bbox_inches="tight")
        print(f"  Saved: {p}")
    plt.close(fig)
...
def main() -> None:
    ...
    print("=" * 70)
    outputs = [
        "forward_probability_map.pdf",
        "forward_risk_map_bau.pdf",
        "forward_scenario_moderate.pdf",
        "forward_scenario_30x30.pdf",
