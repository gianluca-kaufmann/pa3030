#!/usr/bin/env python3
"""Stage 0b: Deployment Model Training (annual hazard, 2001-2024) — USA Model 2 LightGBM.
...
Key differences from model2_LGBM (evaluation model):
...
Outputs:
  data/usa/ml/models/model2_lgbm_deployment_{timestamp}.pkl
  ...
  outputs/usa/results/forward/model2_lgbm_deployment_{timestamp}.txt
...
def resolve_best_params_json() -> Optional[Path]:
    ...
    if scratch is not None:
        candidates += [
            scratch / "scripts/regions/usa/5_training/lgbm_best_params.json",
            scratch / "lgbm_best_params.json",
        ]
    candidates += [
        repo_root / "scripts/regions/usa/5_training/lgbm_best_params.json",
    ]
    for cand in candidates:
        if cand.exists():
            return cand
...
class _ParquetFeatureSequence(getattr(lgb, "Sequence", object)):
    ...
    Implements LightGBM's row-based `lgb.Sequence` contract:
    - `len(seq)` = number of rows
    - `seq[start:stop]` = dense matrix for rows `[start, stop)`
    """
...
def main() -> None:
    ...
    print("=" * 70)
    print(f"MODEL 2 LGBM — DEPLOYMENT MODEL (annual hazard, {DEPLOY_TRAIN_YEARS[0]}-{DEPLOY_TRAIN_YEARS[1]})")
    print("=" * 70)
    ...
    check_feature_denylist(feature_cols, context="usa/lgbm/deployment")
    ...
    model_path = model_dir / f"model2_lgbm_deployment_{timestamp}.pkl"
...
if __name__ == "__main__":
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_path = output_dir / f"model2_lgbm_deployment_{timestamp}.txt"
    tee = Tee(log_path)
    sys.stdout = tee
    try:
        main()
